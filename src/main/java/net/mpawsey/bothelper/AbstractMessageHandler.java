...
public abstract class AbstractMessageHandler extends ListenerAdapter
{
	...
	@Command("admin")
	@CommandDescription(usage="admin [role]", description="Sets the admin role. Admins can use all commands locked behind a permission.")
	public final void adminCommand(@CmdParam(Type.ARGS) String[] args, @CmdParam(Type.CHANNEL) MessageChannel channel, @CmdParam(Type.GUILD) Guild guild)
	{
		if (args.length != 1)
			MessageHelper.sendError("You must provide exactly one admin role.", channel, messageLife);
		else
		{
			long adminRole = MessageHelper.extractId(args[0]);
			if (!FileManager.doesDirectoryExist("admin"))
				new File("admin").mkdir();
			FileManager.writeFile("admins/" + guild.getId(), String.valueOf(adminRole), false);
			MessageHelper.sendSuccess("Updated admin role.", channel, messageLife);
		}
	}
	
	@Command("help")
	@CommandDescription(usage="help [command]", description="Shows all commands or the description of the given command.")
	public final void helpCommand(@CmdParam(Type.CHANNEL) MessageChannel channel, @CmdParam(Type.ARGS) String[] args, @CmdParam(Type.AUTHOR_USER) User user, @CmdParam(Type.AUTHOR_MEMBER) Member member)
	{
		if (args.length > 1)
		...
		else if (args.length == 1)
		{
			...
			{
				if (cmds.get(args[0]).hasPerm(member))
				{
		...
		else
		{
			...
			{
				if (e.getKey().equals("admin") && !member.isOwner())
					continue;
				if (e.getValue().hasPerm(member))
				{
	...
	private class MethodWrapper
	{
		...
		public static long getAdminRole(Guild guild)
		{
			if (FileManager.doesFileExist("admins/" + guild.getId()))
			{
				return Long.parseLong(FileManager.readFile("admins/" + guild.getId()).get(0));
			}
			return 0l;
		}
		
		public boolean hasPerm(Member member)
		{
			if (member.getRoles().stream().anyMatch(x -> x.getIdLong() == getAdminRole(member.getGuild())))
				return true;
			
			for (int i : perms)
			{
				if (!FileManager.hasPermission(perms[i], member.getUser().getId()))
					return false;
			}
			return true;
		}
